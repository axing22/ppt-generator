...
export async function POST(request: Request) {
  try {
    ...
    // 构建解析提示词 (优化版本，更简洁)
    const prompt = `将以下文本解析为PPT幻灯片，返回JSON数组格式。
...
JSON格式：
[
  {"title": "标题", "coreIdea": "核心观点", "arguments": ["论据1", "论据2", "论据3"]}
]
...
只返回JSON，不要其他内容。`;
      ...
      try {
        ...
        if (proxyResponse.ok) {
          ...
          if (proxyData.success && proxyData.data?.choices?.[0]?.message?.content) {
            ...
            try {
              let slides = JSON.parse(aiResponse);
              ...
              const validatedSlides = slides.map((slide: any, index: number) => {
                const validatedSlide: Slide = {
                  ...
                  arguments: Array.isArray(slide.arguments) ? slide.arguments.filter((arg: any) => arg && arg.trim()) : []
                };
    ...
    const validatedSlides = slides.map((slide, index) => {
      const validatedSlide: Slide = {
        ...
        coreIdea: slide.coreIdea || '核心观点',
        arguments: Array.isArray(slide.arguments)
